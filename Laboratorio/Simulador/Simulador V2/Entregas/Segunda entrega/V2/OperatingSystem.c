...
void OperatingSystem_HandleSystemCall() {
	...
	switch (systemCallID) {
		...
		case SYSCALL_SLEEP:
			...
			// Para bloquear un proceso en ejecucion tenemos que sacarlo de su ejecucion => guardar el contexto ...
			PIDABloquear = executingProcessID;
			processTable[PIDABloquear].state = BLOCKED; // asignamos su estado como bloqueado
			OperatingSystem_SaveContext(PIDABloquear); // guardamos su contexto

			OperatingSystem_MoveToTheBlockedState(PIDABloquear); // funcion para agregar en orden ascendente a la cola de procesos dormidos

			int PIDDelRelevador = OperatingSystem_ShortTermScheduler(); // selecciono el proceso que pasara a ejecucion
			OperatingSystem_Dispatch(PIDDelRelevador); // pongo en ejecucion el proceso seleccionado por el planificador a corto plazo
...
void OperatingSystem_HandleClockInterrupt()
{
	...
	{
		int PIDProceso = sleepingProcessesQueue[i].info;
		if(processTable[PIDProceso].whenToWakeUp == numberOfClockInterrupts)
		{
...
void OperatingSystem_MoveToTheBlockedState(int PidProcesoInsertar)
{
	...
	{
		/* El valor del campo whenToWakeUp se obtendrá sumando: un “retardo” al número de interrupciones de reloj que se hayan producido hasta el momento; más una unidad
		adicional, para garantizar que se tiene que despertar en el futuro: whenToWakeUp=retardo+numInterrup+1 */
		int retardo = 0;
		...
		numberOfSleepingProcesses++; // aumentamos el numero de procesos bloqueados en la cola de procesos bloqueados
	}
}
// Fin V2-ej5(f)
