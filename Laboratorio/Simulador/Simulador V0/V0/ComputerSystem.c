...
void ComputerSystem_PowerOn(int argc, char *argv[]) {
  int i, nm;
	...
	programNameFile= fopen(PROGRAM_TO_BE_EXECUTED, "r");
	
	// Check if programFile exists, if not, poweroff system
	if (programNameFile==NULL)
		ComputerSystem_PowerOff();

	FILE *programFile;
	char characters[LINEMAXIMUMLENGTH];
	
	fgets(characters, LINEMAXIMUMLENGTH, programNameFile);
	characters[strcspn(characters, "\n")] = 0; // devuelve el primer caracter de nueva linea (\n) y lo sustituimos por el fin de cadena (\0) para eliminar dicho \n y asi solo tener el nombre del programa a ejecutar situado en la primera linea del fichero nameOfTheProgramToBeExecuted
	programFile= fopen(characters, "r");

	if (programFile==NULL)
		programFile= fopen("programToBeExecuted", "r");
