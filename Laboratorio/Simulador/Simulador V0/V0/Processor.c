...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		// Instruction MEMADD
		case MEMADD_INST:
			registerMAR_CPU = operand2; // guarda la dir de memoria a leer el contenido

			Buses_write_AddressBus_From_To(CPU, MAINMEMORY); // envia a controlador de memoria la direccion a leer mediante los buses

			registerCTRL_CPU = CTRLREAD; // le decimos al controlador de memoria la operacion que queremos realizar

			Buses_write_ControlBus_From_To(CPU,MAINMEMORY); // envia al controlador de memoria la orden que queremos realizar (en este caso una lectura)

			registerAccumulator_CPU = operand1 + registerMBR_CPU.cell; // operand1 + contenido de la celda de memoria indicada por memAddress
			registerPC_CPU++;
			break;
