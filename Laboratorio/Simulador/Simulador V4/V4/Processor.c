...
// Inicio V2-ej5(d)
int registerD_CPU; // definimos un nuevo registro
// Fin V2-ej5(d)
...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		case TRAP_INST: 
			...
			registerPC_CPU++;
			// Inicio V2-ej5(e)
			registerD_CPU=operand2; // modificamos la instruccion TRAP para que se guarde en el registro D el valor de operand2
			// Fin V2-ej5(e)
			break;
		...
		case HALT_INST: 
			// Inicio V1-ej15(a)
			// La intruccion solo va a ser ejecutada si el procesador esta en modo protegido. En caso contrario, se eleva una interrupcion de tipo excepcion
			if(Processor_PSW_BitState(EXECUTION_MODE_BIT) != 0) // si el modo de ejecucion es protegido (!= 0, el 0 representa el modo usuario)...
			{
				// se ejecuta instruccion
				Processor_ActivatePSW_Bit(POWEROFF_BIT);
			} else // si no es protegido el modo de ejecucion...
			{
				// se eleva una interrupcion de tipo excepcion
		...
		// Inicio V1-ej0
		case MEMADD_INST:
			registerMAR_CPU = operand2; // la direccion de la que leemos es la indicada en el segundo operando
			Buses_write_AddressBus_From_To(CPU, MMU); // enviamos la direccion de la que leer a la MMU por los buses
			registerCTRL_CPU = CTRLREAD;
			Buses_write_ControlBus_From_To(CPU, MMU); // enviamos la operacion a realizar en la direccion de memoria enviada
			registerAccumulator_CPU = operand1 + registerMBR_CPU.cell; // asignamos la suma del primer operando y el valor leido en la direccion de memoria indicada en el segundo operando al registro acumulador
			Processor_CheckOverflow(operand1, registerMBR_CPU.cell, REGISTERACCUMULATOR_CPU); // check accumulator
			registerPC_CPU++;
			break;
		// Fin V1-ej0
...
void Processor_ManageInterrupts() {
	...
	int i;
	// Inicio V2-ej2(c)
	if(Processor_PSW_BitState(INTERRUPT_MASKED_BIT)) // si el bit del psw del enmascarado está a 1 => no se comprueban las posibles siguientes interrupciones activas...
	{
		return;	
	} else
	{
...
// Inicio V2-ej5(d)
int Processor_GetRegisterD() // hacemos la funcion que nos permita acceder al valor de este registro D
{
