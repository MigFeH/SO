...
// and OS code and the system stack occupies 60 positions 
	...
	// Inicio V4-ej5(b)
	numeroDeParticiones = OperatingSystem_InitializePartitionTable(); // hacemos la llamada, en la inicializacion del SO, a esta funcion antes de que el SO considere la creacion de proceso alguno
	// Fin V4-ej5(b)
...
int OperatingSystem_CreateProcess(int indexOfExecutableProgram) {
	...
	// Inicio V4-ej6(c)
 	// loadingPhysicalAddress=OperatingSystem_ObtainMainMemory(processSize, PID); // Obtain enough memory space
	int indiceParticionAsignada = OperatingSystem_ObtainMainMemory(processSize, PID, executableProgram->executableName); // le pasamos el nombre del programa ejecutable para poder mostrarlo en el mensaje de intento de obtencion de memoria

	if(indiceParticionAsignada == MEMORYFULL)
	{
		return MEMORYFULL;
	} else if(indiceParticionAsignada == TOOBIGPROCESS)
	{
		return TOOBIGPROCESS;
	}
	loadingPhysicalAddress = partitionsTable[indiceParticionAsignada].initAddress;
	// Fin V4-ej6(c)
	...
	// Inicio V1-ej10
	//ComputerSystem_DebugMessage(TIMED_MESSAGE, 111,SYSPROC, PID, executableProgram->executableName); // ESTA LLAMADA ESTA DENTRO DE LA INICIALIZACION DEL PCB
	// Fin V1-ej10


	// ------------------- SI LA EJECUCIÓN LLEGÓ A ESTA PARTE DE LA FUNCIÓN ES PORQUE EL PROCESO SE CREÓ CORRECTAMENTE ------------------- //

	// Inicio V4-ej6(c)
	partitionsTable[indiceParticionAsignada].PID = PID; // asignamos a la particion el proceso creado
	// Fin V4-ej6(c)

	// Inicio V4-ej8
	processTable[PID].partitionIndex = indiceParticionAsignada; // le asignamos al proceso el indice de la particion en la que lo situamos
	// Fin V4-ej8 
	
	// Inicio V4-ej6(c)
	ComputerSystem_DebugMessage(TIMED_MESSAGE,143,SYSMEM,indiceParticionAsignada,loadingPhysicalAddress,partitionsTable[indiceParticionAsignada].size,PID,executableProgram->executableName);
	// Fin V4-ej6(c)

	// Inicio V4-ej7
	OperatingSystem_ShowPartitionTable("after allocating memory"); // la invocamos despues de la inicializacion del PCB para que al imprimir la informacion de la particion que ocupa el proceso no se produzca segmentation fault
	// Fin V4-ej7
...
int OperatingSystem_ObtainMainMemory(int processSize, int PID, char* programName) {
// Fin V4-ej6(b)
	// Inicio V4-ej6(a)

	// Inicio V4-ej6(b)
	ComputerSystem_DebugMessage(TIMED_MESSAGE,142,SYSMEM,PID,programName,processSize);
	// Fin V4-ej6(b)
	...
	/* Elige una partición para el proceso siguiendo la política (estrategia) del mejor ajuste;
	 * Si hubiese varias, se debe elegir la que empiece en una dirección de memoria más baja. 
	 *
	 * El proceso solicita una cantidad de memoria superior a la de cualquier partición => La función deberá retornar un error TOOBIGPROCESS.
	 * El proceso cabe en alguna partición, pero todas las particiones con tamaño suficiente están ocupadas => La función deberá retornar un error MEMORYFULL.
	 */

	int cotaTamParticion = partitionsTable[0].size;
	int indiceParticionElegida = -1;

	int numeroDeVecesDeExcesosDeTam = 0; // si el proceso no cabe en la particion se incrementa el valor => si el valor final es el mismo que el del numero de particiones => TOOBIGPROCESS
	int numeroDeParticionesLibresCandidatas = 0; // si el proceso cabe en la particion y dicha particion está libre => se incrementa el valor. Si el valor final es de 0 => MEMORYFULL

	for(int i = 0; i < numeroDeParticiones; i++)
	{
		int tamParticion = partitionsTable[i].size;

		if(tamParticion >= processSize) // Proceso cabe en particion
		{
			if(partitionsTable[i].PID == NOPROCESS) // particion libre
			{
				if(tamParticion < cotaTamParticion /* el tam de la particion se ajusta mejor... */ || numeroDeParticionesLibresCandidatas == 0 /* es la primera particion encontrada... */)
	 			{
	 				indiceParticionElegida = i;
	 				cotaTamParticion = tamParticion;
	 			}

				numeroDeParticionesLibresCandidatas++;
			}
		} else // Proceso no cabe en particion
		{
			numeroDeVecesDeExcesosDeTam++;
		}
	}

	if(numeroDeVecesDeExcesosDeTam == numeroDeParticiones)
	{
		return TOOBIGPROCESS;
	} else
	{
		if(numeroDeParticionesLibresCandidatas == 0)
		{
			return MEMORYFULL;
		} else
		{
			return indiceParticionElegida;
		}
	}
	// Fin V4-ej6(a)
}
...
void OperatingSystem_PCBInitialization(int PID, int initialPhysicalAddress, int processSize, int priority, int processPLIndex) {
	...
	// Inicio V4-ej8
	processTable[PID].partitionIndex = -1;
	// Fin V4-ej8
}
...
void OperatingSystem_HandleException() {
	...
	switch(Processor_GetRegisterD())
	{
		...
		case INVALIDPROCESSORMODE:
			causa = "invalid processor mode";
			break;
		case INVALIDADDRESS:
			causa = "invalid address";
			break;

		// Inicio V4-ej3(b)
		default: // INVALIDINSTRUCTION
			causa = "invalid instruction";
			break;
		// Fin V4-ej3(b)
	}

	ComputerSystem_DebugMessage(TIMED_MESSAGE,140,INTERRUPT,executingProcessID,programList[processTable[executingProcessID].programListIndex]->executableName,causa);
...
void OperatingSystem_HandleSystemCall() {
	...
	switch (systemCallID) {
		...
		// Inicio V4-ej4(a)
		default: // llamada al sistema con identificador no existente...
			// mostramos mensaje
			...
			// Inicio V4-ej4(b)
			OperatingSystem_TerminateExecutingProcess(); // hacemos que el proceso en ejecucion termine
			// Fin V4-ej4(b)
...
void OperatingSystem_HandleClockInterrupt()
{
	...
	// Inicio V3-ej3(a)
	int res = OperatingSystem_LongTermScheduler();
	// Fin V3-ej3(a)

	// Inicio V3-ej4(b)
	if(numberOfNotTerminatedUserProcesses == 0) // condicion que detiene la simulacion tras ejecutarse el PLP 
	{
		// finalizamos la simulacion
		OperatingSystem_ReadyToShutdown();
	}
	...
	// Inicio V2-ej6(b)
	if(contadorDesbloqueos || res > 0 /* V3-ej3(b) */) // si el contador de desbloqueos es mayor que 0...
	{
...
void OperatingSystem_ReleaseMainMemory()
{
	/* Libera la partición de memoria ocupada por el proceso en ejecucion. La función deberá mostrar un mensaje como el siguiente. */

	OperatingSystem_ShowPartitionTable("before releasing memory");

	int indiceDeLaParticionDelProceso = processTable[executingProcessID].partitionIndex;

	partitionsTable[indiceDeLaParticionDelProceso].PID = NOPROCESS;
	ComputerSystem_DebugMessage(TIMED_MESSAGE,145,SYSMEM,indiceDeLaParticionDelProceso,partitionsTable[indiceDeLaParticionDelProceso].initAddress,partitionsTable[indiceDeLaParticionDelProceso].size,executingProcessID,programList[processTable[executingProcessID].programListIndex]->executableName);
